...
class Menu:
    def __init__(self, items: list[MenuItem], sound_manager: SoundManager = None):
        self.items = items
    ...
    def move_up(self):
        ...
        while steps < len(self.items):
            ...
            if self.items[self.current_index].enabled:
                if self.sound:
                    self.sound.play("move", volume=1.0)
                break
            steps += 1
    ...
    def handle_action(self, action: str) -> bool:
        if action == "up":
            ...
            if self.sound:
                self.sound.play("select", volume=1.0)
...
if __name__ == "__main__":
    ...
    sound.play_music("src/sounds/undertale-start-menu.wav", volume=0.25)
